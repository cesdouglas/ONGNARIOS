				...
				<section class="6u 12u(narrower)">
					<div class="box post">
						...
						<div class="inner">
							<h3>${n.nm_vaga}</h3>
							<p>
								Número de vagas: ${n.nr_vaga}<br> Descrição: ${n.ds_vaga}<br>
								Salário: ${n.vl_salario}<br> CNPJ:
								${n.t_ONG_EMPRESA_nr_cnpj}
							</p>
							<c:if test="${!empty usuario.nr_cpf}">
								...
								<c:if test="${session.nr_cpf == n.t_ONG_USUARIO_nr_cpf}">
									<form action="midlet" method="post">
								...
								<c:if
									test="${!empty n.t_ONG_USUARIO_nr_cpf && usuario.nr_cpf != n.t_ONG_USUARIO_nr_cpf}">
									<form action="midlet" method="post">
										<input type="hidden" name="form" value="inscrever"> <input
											type="submit" style="background-color: #37c0fb"
											value="Inscreva-se">
